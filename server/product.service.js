...
async function getLastRegistrator() {
  ...
  try {
    ...
    console.log(
      'server/product.service.js - getLastRegistrator ' + error.stack
    );
    logger.error(
      'server/product.service.js - getLastRegistrator ' + error.stack
    );
...
export async function getProductsService(parameters) {
  ...
  try {
    ...
    if (parameters.maxPrice < parameters.minPrice) {
      logger.error(
        'server/product.service.js - getProductsService, parameter Price min > max'
      );
      console.log(
        'server/product.service.js - getProductsService, parameter Price min > max'
      );
    ...
    if (
      ...
      logger.error(
        'server/product.service.js - getProductsService, parameter Price is not two numbers'
      );
      console.log(
        'server/product.service.js - getProductsService, parameter Price is not two numbers'
      );
    ...
    if (
      parameters.product_group !== undefined &&
      parameters.product_group.length != 0
    ) {
      query1.where.product_group_id = { in: parameters.product_group };
    }
    if (parameters.search_name) {
      query1.where.product_name = {
        contains: parameters.search_name,
        mode: 'insensitive',
      };
    }
    if (
      parameters.vid_modeli !== undefined &&
      parameters.vid_modeli.length != 0
    ) {
    ...
    if (res_qnt_price.length > 0) {
      ...
      for (const element_group of qnt_price_group) {
        ...
        for (const element of res_qnt_price) {
          // перебираем полный массив, чтобы заполнить сгруппированный массив
          ...
          if (element.product.image_registry) {
            element_group.image_active_path =
              element.product.image_registry.find(
                (e) => e.main === true
              )?.full_name;
          }
          element_group.create_date = element.product.create_date;
          element_group.qnt_price.push({
            // вложенный объект с ценами
      ...
      for (const element_group of qnt_price_group) {
        if (element_group.qnt_price.length > 0) {
          // во вложенном объекте с ценами группируем повторяющиеся размеры одного продукта
          element_group.qnt_price = groupAndSum(
            element_group.qnt_price,
            ['size', 'sum'],
            ['qnt'],
            ['store_id']
    ...
    return {
      data: qnt_price_group,
      full_count: res_count.length,
      current_count: qnt_price_group.length,
    };
  } catch (error) {
    logger.error(
      'server/product.service.js - getProductsService ' + error.stack
    );
...
export async function getProductsFiltersService() {
  ...
  if (!registrator_id) {
    logger.info(
      'server/product.service.js - getProductsFilters ended not registrator'
    );
    return obj_all;
  }

  async function getTables(field) {
    // возвращает сгруппированнные записи регистра по заданному полю
    try {
      const res = await prismaI.qnt_price_registry.groupBy({
        by: [field],
        orderBy: {
      ...
      console.log(
        'server/product.service.js - getProductsFilters ' + error.stack
      );
      logger.error(
        'server/product.service.js - getProductsFilters ' + error.stack
      );
  ...
  obj_all.product_group = await getNames(
    'product_group',
    res_product_group,
    'name_1c'
  );
  ...
  res_vid_modeli = res_vid_modeli.filter((e) => {
    // удаляем null так как поле Вид модели не обязательное
    if (e.vid_modeli_id !== null) {
      return e;
    }
  });
...
export async function getProductService(product_id, name_1c) {
  ...
  try {
    ...
    if (res instanceof Object) {
      if (Object.hasOwn(res, 'qnt_price_registry')) {
        if (res.qnt_price_registry.length > 0) {
          res.qnt_price_registry_group = groupAndSum(
    ...
    logger.error(
      'server/product.service.js - getProductService ' + error.stack
    );
  }
...
export async function getProductsNewsService(news) {
  ...
  try {
    ...
    let query1 = {
      // выбираем последние продукты по дате создания
      orderBy: {
        create_date: 'desc',
      },
      where: {
    ...
    let query11 = {
      // группируем по продуктам и ценам
      select: {
        product_id: true,
        product_name: true,
        sum: true,
    ...
    let query2 = {
      // делаем запрос по остаткам и ценам
      include: {
        product_group: {
          select: {
            id: true,
    ...
    for (const element_group of res11) {
      ...
      for (const element of res2) {
        ...
        if (element.product.image_registry) {
          element_group.image_registry = element.product.image_registry;
          element_group.image_active_path = element.product.image_registry.find(
            (e) => e.main === true
          ).full_name;
        }
    ...
    for (const element_group of res11) {
      if (Object.hasOwn(element_group, 'qnt_price')) {
        // во вложенном объекте с ценами группируем повторяющиеся размеры одного продукта
        element_group.qnt_price = groupAndSum(
          element_group.qnt_price,
          ['size', 'sum'],
    ...
    console.log(
      'server/product.service.js - getProductNewsService ' + error.stack
    );
    logger.error(
      'server/product.service.js - getProductNewsService ' + error.stack
    );
